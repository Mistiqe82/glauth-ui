...
class UserView(MyModelView):
    ...
    def valid_chars(self, field):
        ...
        if result:
            raise ValidationError(
                '"{}" is not allowed'.format(result.group(0)))
    ...
    column_list = ('username', 'givenname', 'surname', 'mail',
                   'unixid', 'is_active', 'pgroup')  # , 'othergroups')
    # Configure columns that are editable in list view
    column_editable_list = ['username', 'mail',
                            'givenname', 'surname', 'is_active']
    # Configure columns that a searchable
    column_searchable_list = column_editable_list
    # Configure columns to exclude in list view
    column_exclude_list = ['password_hash']
    # Details View List
    column_details_list = ('username', 'givenname',
                           'surname', 'mail', 'unixid', 'is_active', 'pgroup')
    ...
    form_args = dict(
        ...
        username=dict(validators=[valid_chars, no_root_allowed, Length(
    ...
    def on_model_change(self, form, model, is_created):
        ...
        if is_created:
            if form.send_invite_link.data and (
                    (not form.mail.data) or form.mail.data == ''):
                raise ValidationError(
            ...
            if hasattr(form, 'send_pw_reset_link'):
                if form.send_pw_reset_link.data and (
                        (not form.mail.data) or form.mail.data == ''):
                    raise ValidationError(
                ...
                if form.send_pw_reset_link.data:
                    ...
                    flash(
                        'Reset Password Link was send to  {}'.format(
                            model.mail))
...
class GroupView(MyModelView):
    ...
    column_details_list = ('name', 'unixid', 'primary',
    ...
    form_args = dict(
        name=dict(
            validators=[
                ...
                Length(
                    min=3,
                    message='Group names have to be atleast 3* characters long')]),
    )

    # Configure which form fields to show
    form_columns = ('name', 'unixid', 'primary', 'description',
                    'includes', 'included_in', 'p_users', 'o_users')
    ...
    def create_form(self, obj=None):
        ...
        default_unixid = 5500
        highest_group = Group.query.order_by(
            Group.unixid.desc()).limit(1).all()
        if highest_group:
...
# Customize Base and AdminIndex View with custom is accessible rules
    ...
    def index(self):
        ...
        if form.validate_on_submit():
            ...
            try:
                ...
                flash(
                    'Glauth settings NOT updated, an error occured: ' +
                    str(exc))
...
class AdminHomeView(MyAdminIndexView):
    ...
    def index(self):
        counts = {
            'users': '{}'.format(
                User.query.count()), 'pgroups': '{}'.format(
                Group.query.filter_by(
                    primary=True).count()), 'ogroups': '{}'.format(
                    Group.query.filter_by(
                        primary=False).count())}
...
admin = Admin(app,
              name='{} - Admin'.format(app.config['APPNAME']),
              template_mode='bootstrap4',
              index_view=AdminHomeView(menu_icon_type='fa',
                                       menu_icon_value='fa-home'))

# Add model views
admin.add_view(UserView(User, db.session, menu_icon_type='fa',
               menu_icon_value='fa-users', name="Users"))
admin.add_view(GroupView(Group, db.session, menu_icon_type='fa',
               menu_icon_value='fa-server', name="Groups"))
admin.add_view(GlauthConfig(name="Settings",
               menu_icon_type='fa', menu_icon_value='fa-cog'))
admin.add_view(LeaveAdmin(name="Exit", menu_icon_type='fa',
               menu_icon_value='fa-arrow-circle-right'))
