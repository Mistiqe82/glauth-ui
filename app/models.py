...
class Group(db.Model):
    ...
    included_in = db.relationship(
        'Group', secondary=included_groups, primaryjoin=(
            included_groups.c.include_id == unixid), secondaryjoin=(
            included_groups.c.included_in_id == unixid), backref=db.backref(
                'includes', lazy='dynamic'), lazy='dynamic')
...
class User(UserMixin, db.Model):
    ...
    # add activation date?
    primarygroup = db.Column(db.Integer, db.ForeignKey(
        'group.unixid'), nullable=False)
    othergroups = db.relationship(
        'Group',
        secondary=othergroups_users,
        backref=db.backref(
            'o_users',
            lazy='dynamic'))
    ...
    @property
    def is_admin(self):
        # checks if the name of any group matches the configured ADMIN_GROUP
        # name
        if [group for group in self.othergroups if group.name ==
...
def create_basic_db():
    ...
    og1 = Group(name='glauth_admin', unixid=5551,
                description='Glauth UI admin group')
    og2 = Group(name='vpn', unixid=5552)

    db.session.add(og1)
    db.session.add(og2)

    pg1 = Group(name='people', unixid=5501, primary=True,
                description='primary user group', includes=[og2])
    pg2 = Group(name='svcaccts', unixid=5502, primary=True,
    ...
    u1 = User(
        ...
        surname='Doe',
        unixid=5001,
        password_hash='6478579e37aff45f013e14eeb30b3cc56c72ccdc310123bcdf53e0333e3f416a',
    ...
    u2 = User(
        username='search',
        unixid=5002,
        password_hash='125844054e30fabcd4182ae69c9d7b38b58d63c067be10ab5ab883d658383316',
        pgroup=pg2)
    # PW: searchpw
    db.session.add(u1)
    db.session.add(u2)
