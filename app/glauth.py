...
def create_glauth_config():
    ...
    new_config += "\n[ldaps]\n"
    new_config += "  enabled = {}\n".format(
        str(settings.ldaps_enabled).lower())
    ...
    new_config += "  AnonymousDSE = {}\n".format(
        str(settings.anonymousDSE).lower())
    ...
    for user in users:
        ...
        if len(user.othergroups) > 0:
            new_config += "  otherGroups = [ {} ]\n".format(
                ",".join(str(group.unixid) for group in user.othergroups))
    ...
    for group in groups:
        ...
        if group.included_in.count() > 0:
            new_config += "  includegroups = [ {} ]\n".format(
                ",".join(str(group.unixid) for group in group.included_in))
