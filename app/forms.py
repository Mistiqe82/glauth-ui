...
class EditProfileForm(FlaskForm):
    givenname = StringField('Givenname', validators=[
...
class NewAccountForm(FlaskForm):
    ...
    password2 = PasswordField(
        'Repeat Password', validators=[
            DataRequired(), Length(
                min=6, max=20), EqualTo('password')], render_kw={
            'autocomplete': 'new-password'})
...
class EditGlauthForm(FlaskForm):
    ...
    ldap_listen = StringField('Address and Port', render_kw={
                              "placeholder": "0.0.0.0:389"})
    ldaps_enabled = BooleanField('Enable LDAPS')
    ldaps_listen = StringField('Address and Port', render_kw={
                               "placeholder": "0.0.0.0:636"})
    ldaps_cert = StringField(
        'Certificate', render_kw={
            "placeholder": "/path/to/server.crt"})  # Required if ldaps_enabled
    # Required if ldaps_enabled
    ldaps_key = StringField(
        'Key', render_kw={"placeholder": "/path/to/server.key"})
    basedn = StringField('BaseDN', render_kw={
                         "placeholder": "dc=glauth,dc=com"})  # Data Required
    ...
    groupformat = StringField(
        ...
        validators=[
            Length(
                max=2)],
        description='Configure to customize dn format.')
    sshkeyattr = StringField('SSH-Key Attribute',
                             render_kw={"placeholder": "sshPublicKey"},
                             description='Configure ssh-key attribute name.')
    anonymousDSE = BooleanField(
        'AnonymousDSE', description='Allow AnonymousDSE, useful for SSSD')
